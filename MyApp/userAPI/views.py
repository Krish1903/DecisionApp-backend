...
class ActivePollsFromFollowedUsersView(APIView):
    def get(self, request, user_id, format=None):
        ...
        followed_users = [
            ...
            if ua.user != user_account.user and ua.user not in blocked_users_list
        ]
...
class UserSearchView(APIView):
    def get(self, request, search_string=None, *args, **kwargs):
        ...
        if current_user.is_authenticated:
            ...
            blocked_by_users = User.objects.filter(useraccount__blocked_users=current_user.useraccount).values_list('id', flat=True)
